...
# 🟢 修复后的版本：移除了 ODIN/Mahalanobis 的引用，专用于 EMP
def evaluate_detect(model, dataset_ind, dataset_ood, criterion, eval_func, args, device, return_score=False):
    ...
    if isinstance(dataset_ood, list):
        ...
        # 🔴 可视化数据保存
        try:
            np.save('energy_id.npy', test_ind_score.cpu().numpy())
            np.save('energy_ood.npy', test_ood_score.cpu().numpy())
            # print("💾 Visualization data saved: energy_id.npy, energy_ood.npy")
    ...
    # 3. 计算 IND 数据的分类准确率和 Validation Loss
    with torch.no_grad():
